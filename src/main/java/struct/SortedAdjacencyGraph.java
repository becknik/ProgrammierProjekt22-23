...
public class SortedAdjacencyGraph {
	...
	public SortedAdjacencyGraph (final AdjacencyGraph adjacencyGraph) {
		...
		//adding every node from adjacencyGraph to the sortedAdjacencyGraph as node record.
	...
	private int getNearestNodeId (final double longitude, final double latitude, final int initialNearestNodeIndex, final MovementDirection direction) {
		...
		// For loop searches the left/right side of sorted array (depending on crementer) of initial node index
		// for nodes with better distance to specified coords
	 ...
	 * @param fromLatitude the latitude value of a given position
	...
	private boolean isOutOfBounds (final int index) {
		return index < 0 || this.nodeIdsSortedByLatitude.length <= index;
	}
